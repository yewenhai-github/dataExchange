...
import com.easy.query.SQLProcer;
...
public class MainServlet extends HttpServlet {
	...
	public MainServlet(){
		super();
		CheckLogin = true;
		if("false".equals(GetSetting("system", "CheckLogin"))){
			CheckLogin = false;
		}
		LoginPage = GetSetting("system", "LoginPage");
		if (SysUtility.isEmpty(LoginPage)) {
			LoginPage = "~/Login.html";
		}
		StartPage = GetSetting("system", "StartPage");
		if (SysUtility.isEmpty(StartPage)) {
	...
	private void Init(HttpServletRequest request, HttpServletResponse response)throws IOException ,LegendException, JSONException {
		...
		SessionManager.setCheckLogin(CheckLogin);
		SessionManager.setLoginPage(LoginPage);
		SessionManager.setStartPage(StartPage);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		SessionManager.setIsPost(false);
		try {
			Check(request, response);
		} catch (Exception e) {
			LogUtil.printLog("doGet请求错误！"+SysUtility.getStackTrace(e), Level.ERROR);
		} finally {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		SessionManager.setIsPost(true);
		try {
			Check(request, response);
		} catch (Exception e) {
			LogUtil.printLog("doPost请求错误！"+SysUtility.getStackTrace(e), Level.ERROR);
		} finally {
			SysUtility.closeActiveCN(SysUtility.getCurrentConnection());
			SessionManager.destorySession();
		}
		...
		if (cando) {
			try {
				if(SysUtility.isNotEmpty(SessionManager.getDataAccess()) || SysUtility.getDBPoolClose()){
					LogUtil.printLog("当前线程类名："+getServletName(), Level.INFO);
