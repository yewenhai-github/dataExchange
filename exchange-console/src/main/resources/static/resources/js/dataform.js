...
function DoCommand(cmd, commanddata) {
    ...
    if (commanddata != '' && commanddata != undefined) {
        postData(cmd, "CommandData=" + encodeURIComponent(commanddata), true, DataReturns, onerror);
    }
    else {
        ...
        if (vf != undefined && vf != "") {
            ...
            if (v != undefined) {
                if ($(inputs[e]).attr("type") == "hidden") {
                    $(inputs[e]).val(UntransferString(v.toString()));
                }
                else {
                    $(inputs[e]).textbox("setText", UntransferString(v.toString()));
                }
            }
        }
    }
...
function DataReturns(data, proc) {
    ...
    if (parent.document.getElementById('iframeHright') != null) {
        if (parent.document.getElementById('iframeHright').contentWindow.$('#dg').length > 0) {
...
function Hide(hideclass, btnid, datagrid) {
    $("." + hideclass).hide();
...
//列表数据绑定
function GridDataBindGet(container, parentPoint, SelectID) {
    var p = $("#" + container);
    p.datagrid({
        type: "GET",
        url: parentPoint + 'GetGridDataUI?SelectID=' + SelectID,
        dataType: "json",
        fit: true,
        striped: true,
        fitColumns: false,
...
//编辑页面数据绑定
function EditDataBind(parentPoint, SelectID, param) {
    var tempValue = GetRequestName(param);
    if (isNotEmpty(tempValue)) {
        $.ajax({
            url: parentPoint + 'GetGridDataUI?SelectID=' + SelectID + '&' + param + '=' + tempValue,
            dataType: "json",
            success: function (data) {
                FormBind(data);
            }
...
//下拉数据绑定
function ComboboxBindGet(container, parentPoint, SelectID, valueField, textField) {
    $("#" + container).combobox({
        method: 'get',
        url: parentPoint + 'GetComboboxDataUI?SelectID=' + SelectID,
...
//字段校验公共方法
//普通输入框
function textBoxCheck(id, msg) {
    if ($('#' + id).val() == null || $('#' + id).val() == "") {
        $.messager.alert("警告!", msg + "不能为空");
        return false;
    }
    return true;
}
...
//下拉框
function comBoBoxCheck(id, msg) {
    if ($("#" + id).combobox('getValue') == null || $("#" + id).combobox('getValue') == "") {
        $.messager.alert("警告!", msg + "不能为空");
        return false;
    }
    return true;
...
function dateCheck(id, msg) {
    if ($('#' + id).datebox('getValue') == null || $('#' + id).datebox('getValue') == "") {
        $.messager.alert("警告!", msg + "不能为空");
        return false;
    }
    return true;
...
//时间框有时分秒
function dateTimeCheck(id, msg) {
    if ($('#' + id).datetimebox('getValue') == null || $('#' + id).datetimebox('getValue') == "") {
        $.messager.alert("警告!", msg + "不能为空");
        return false;
    }
    return true;
...
function photoCheck(id) {
    if ($("#" + id).val() == null
        || $("#" + id).val() == "") {
        $.messager.alert("警告!", "手机不能为空");
        parent.LoadingHide();
        return false;
    } else {
        var reTell = /^1\d{10}$/
        if (!reTell.test($("#" + id).val())) {
            $.messager.alert("警告!", "请输入正确的手机号");
            parent.LoadingHide();
            return false;
        }
    }
...
//证件号校验
function idNoCheck(id) {
    if ($("#" + id).val() == null
        || $("#" + id).val() == "") {
        $.messager.alert("警告!", "证件号不能为空");
        parent.LoadingHide();
        return false;
    } else {
        var reTell = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;  
        if (!reTell.test($("#" + id).val())) {
            $.messager.alert("警告!", "请输入18位的证件号");
            parent.LoadingHide();
            return false;
        }
    }
...
function rowCustomsStatus(val,row,index){
	var rt = "";
	switch (row.customs_satus_code) {
		case '0':
			rt += "<span class=\"label label-default\">草稿</span>";
			break;
		case '1':
			rt += "<span class=\"label label-success\">已申报</span>";
			break;
		case '2':
			rt += "<span class=\"label label-danger\">待抽查</span>";
			break;
		case '3':
			rt += "<span class=\"label label-success\">已放行</span>";
			break;
		case '4':
			rt += "<span class=\"label label-danger\">放行失败</span>";
			break;
		case '5':
			rt += "<span class=\"label label-success\">待放行</span>";
			break;
		default:
...
function rowCheckStatus(val,row,index){
	var rt = "";
	switch (row.check_status_code) {
		case '0':
			rt += "<span class=\"label label-warning\">抽查不通过</span>";
			break;
		case '1':
			rt += "<span class=\"label label-success\">抽查通过</span>";
			break;
		default:
			rt += "<span class=\"label label-warning\"></span>";
		break;
...
function rowCommitStatus(val,row,index){
	var rt = "";
	switch (row.commit_status) {
		case '0':
			rt += "<span class=\"label label-default\">未提交</span>";
			break;
		case '1':
			rt += "<span class=\"label label-success\">已提交</span>";
			break;
		case '2':
			rt += "<span class=\"label label-success\">提交通过</span>";
			break;
		case '3':
			rt += "<span class=\"label label-success\">提交失败</span>";
			break;
		default:
			rt += "<span class=\"label label-warning\"></span>";
		break;
...
function rowConfirmStatus(val,row,index){
	var rt = "";
	switch (row.confirm_status) {
		case '0':
			rt += "<span class=\"label label-default\">未提交</span>";
			break;
		case '1':
			rt += "<span class=\"label label-warning\">已提交</span>";
			break;
		case '2':
			rt += "<span class=\"label label-success\">已确认</span>";
			break;
		default:
			rt += "<span class=\"label label-warning\"></span>";
		break;
...
function rowConfirmStatus1(val,row,index){
	var rt = "";
	switch (row.confirm_status) {
		case '0':
			rt += "<span class=\"label label-default\">未提交</span>";
			break;
		case '1':
			rt += "<span class=\"label label-warning\">待确认</span>";
			break;
		case '2':
			rt += "<span class=\"label label-success\">已确认</span>";
			break;
