...
$(document).ready(function () {
        ...
        $("#corptable").jqGrid({
            ...
            colModel: [
                        ...
                        { name: 'CONFIGNAME', index: 'CONFIGNAME', width: 220,formatter: urlConfig,align : 'center'},//配置名                        
                        { name: 'FUNCTIONTYPENAME', index: 'FUNCTIONTYPENAME', width: 220,align : 'center'},//功能类型
                        { name: 'SOURCEFILETYPE', index: 'SOURCEFILETYPE', width: 220,hidden:true},//源文件类型
                        { name: 'SOURCEFILETYPENAME', index: 'SOURCEFILETYPE', width: 220,formatter: urlCode,align : 'center'},//源文件类型名称
                        { name: 'TARGETFILETYPE', index: 'TARGETFILETYPE', width: 220,hidden:true},//目标文件类型                       
                        { name: 'TARGETFILETYPENAME', index: 'TARGETFILETYPE', width: 220,formatter: urlCode,align : 'center'},//目标文件类型名称                      
                        { name: 'RECEIVETYPE', index: 'RECEIVETYPE', width: 220, hidden:true,align : 'center'},//接收类型
                        { name: 'RECEIVETYPENAME', index: 'RECEIVETYPE', width: 220, formatter: urlCode,align : 'center'},//接收类型
                        { name: 'RECEIVEADRESS', index: 'RECEIVEADRESS', width: 220,align : 'center'},//接收地址
                        { name: 'RECEIVEUSERNAME', index: 'RECEIVEUSERNAME', width: 220,align : 'center'},//接收用户名
                        { name: 'RECEIVEUSERPWD', index: 'RECEIVEUSERPWD', width: 220,align : 'center'},//接收密码
                        { name: 'RECEIVEPATH', index: 'RECEIVEPATH', width: 220,hidden:true},//接收文件存放目录
                        { name: 'SENDTYPE', index: 'SENDTYPE', width: 220, hidden:true,align : 'center'},//发送类型                        
                        { name: 'SENDTYPENAME', index: 'SENDTYPENAME', width: 220, formatter: urlCode,align : 'center'},//发送类型                        
                        { name: 'SENDADRESS', index: 'SENDADRESS', width: 220,align : 'center'},//发送地址
                        { name: 'SENDUSERNAME', index: 'SENDUSERNAME', width: 220,align : 'center'},//发送用户名
                        { name: 'SENDUSERPWD', index: 'SENDUSERPWD', width: 220,align : 'center'},//发送密码
                        { name: 'SENDPATH', index: 'SENDPATH', width: 220,hidden:true},//发送存放目录                        
                        { name: 'USEREMAIL', index: 'USEREMAIL', width: 220,hidden:true},//邮箱                 
                        { name: 'LOCALFILEPATH', index: 'LOCALFILEPATH', width: 220,hidden:true},//本地目录                 
                        { name: 'LOCALFILESWITCH', index: 'LOCALFILESWITCH', width: 220,hidden:true},//开关          
                        { name: 'REMARK', index: 'REMARK', width: 220,align : 'center'}//描述
        	        ],
        	ondblClickRow :function (rowid , iRow, iCol, e){
        		var config = $("#corptable").getRowData(rowid).CONFIGNAME;
        		var configname = $(config).attr("name");
        		parent.NavaddTab("配置信息",
        				"mess/ConfigPathInfo.html?INDX="+$("#corptable").getRowData(rowid).INDX,configname);
        	},
