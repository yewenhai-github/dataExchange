...
@WebServlet("/forms/GetIIDeleteCommonPackType")
public class GetIIDeleteCommonPackType extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;
	public GetIIDeleteCommonPackType()
	{
		SetCheckLogin(false);
	}
	public void DoCommand() throws Exception{
		...
		if (arr.length()>0) {			
			...
			for(int i=0;i<arr.length();i++) {
				Count++;
				boolean isFlag =getDataAccess().ExecSQL("delete from  ITF_DCL_IO_DECL_GOODS_PACK where PACK_ID=" + arr.getJSONObject(i).getString("PACK_ID"));
				if(isFlag){
					StatusCount++;
				}
			}
