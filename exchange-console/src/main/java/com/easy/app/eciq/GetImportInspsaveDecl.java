...
@WebServlet("/forms/GetImportInspsaveDecl")
public class GetImportInspsaveDecl  extends MainServlet{  
	private static final long serialVersionUID = 3594076222956321552L; 
	public GetImportInspsaveDecl()
	{
		SetCheckLogin(false);
	}
	public void DoCommand() throws Exception{ 
		...
		if (InsertRt||updateRt) {
            String DECL_NO1=GetDataValue("DeclData", "DECL_NO");
            SaveToTable("DeclData", "DECL_NO", DECL_NO1);	
            
            //添加的时候，先保存综合流水号，表头保存完成之后，再换成报检流水号
            getDataAccess().ExecSQL("update ITF_DCL_IO_DECL_LIMIT set DECL_NO=? where DECL_NO = ?",new String[] { DECL_NO1,GetDataValue("DeclData", "CON_INDX")});
            getDataAccess().ExecSQL("update ITF_DCL_IO_DECL_ATT set DECL_NO=? where DECL_NO = ?",new String[] { DECL_NO1,GetDataValue("DeclData", "CON_INDX")});
            getDataAccess().ExecSQL("update ITF_DCL_IO_DECL_USER set DECL_NO=? where DECL_NO = ?",new String[] { DECL_NO1,GetDataValue("DeclData", "CON_INDX")});
            getDataAccess().ExecSQL("update ITF_DCL_MARK_LOB set DECL_NO=? where DECL_NO = ?",new String[] { DECL_NO1,GetDataValue("DeclData", "CON_INDX")});
			String SqlTemp=String.format("select * from ITF_DCL_RECEIPTS WHERE DECL_NO = '%s' AND RSP_CODES = '0'",DECL_NO1);
			List lst = SQLExecUtils.query4List(SqlTemp);
	   		int size = lst.size();
	   		if(size==0){
	   			JSONObject tempDecl = new JSONObject();
	   			JSONArray tempArrs = new JSONArray(); 
					tempDecl.put("DECL_NO",DECL_NO1); 
