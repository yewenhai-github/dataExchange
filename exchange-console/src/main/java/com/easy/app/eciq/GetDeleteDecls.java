...
@WebServlet("/forms/GetDeleteDecls")
public class GetDeleteDecls extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;
	public GetDeleteDecls()
	{
		SetCheckLogin(false);
	}
	public void DoCommand() throws Exception{
		...
		if (arr.length()>0) {			
			...
			for(int i=0;i<arr.length();i++) {
				Count++;
				boolean isFlag =getDataAccess().ExecSQL("delete from  ITF_DCL_IO_DECL_GOODS where GOODS_ID=" + arr.getJSONObject(i).getString("GOODS_ID"));
				if(isFlag){
					StatusCount++;
				}
			}
