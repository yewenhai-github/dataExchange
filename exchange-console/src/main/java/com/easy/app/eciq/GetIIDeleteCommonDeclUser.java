...
@WebServlet("/forms/GetIIDeleteCommonDeclUser")
public class GetIIDeleteCommonDeclUser extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;
	public GetIIDeleteCommonDeclUser()
	{
		SetCheckLogin(false);
	}
	public void DoCommand() throws Exception{
		...
		if (arr.length()>0) {			
			...
			for(int i=0;i<arr.length();i++) {
				Count++;
				boolean isFlag =getDataAccess().ExecSQL("delete from  ITF_DCL_IO_DECL_USER where USER_ID=" + arr.getJSONObject(i).getString("USER_ID"));
				if(isFlag){
					StatusCount++;
				}
			}
