...
@WebServlet("/forms/DeleteInspGoodsLimits")
public class DeleteInspGoodsLimits extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;
	public DeleteInspGoodsLimits()
	{
	...
	public void DoCommand() throws Exception{
		...
		if(rows > 0){
			...
			for(int i = 0 ; i < rows ; i ++){
				boolean isFlag =getDataAccess().ExecSQL("delete from  ITF_DCL_IO_DECL_GOODS_LIMIT where LIMIT_ID = ? " ,new String[]{ this.GetDataValue("key", "LIMIT_ID", i)});
				if(isFlag){
					StatusCount++;
				}				
			}
