...
@WebServlet("/forms/GetIIDeleteCommonGoodsCont")
public class GetIIDeleteCommonGoodsCont extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;
	public GetIIDeleteCommonGoodsCont()
	{
		SetCheckLogin(false);
	}
	public void DoCommand() throws Exception{
		...
		if (arr.length()>0) {			
			...
			for(int i=0;i<arr.length();i++) {
				Count++;
				boolean isFlag =getDataAccess().ExecSQL("delete from  ITF_DCL_IO_DECL_GOODS_CONT where GOODS_CONT_ID = ? " , new String[]{ arr.getJSONObject(i).getString("GOODS_CONT_ID")});
				if(isFlag){
					StatusCount++;
				}
			}
