...
@WebServlet("/forms/SaveInspDeclGoodss")
public class SaveInspDeclGoodss extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;
	public SaveInspDeclGoodss()
	{
	...
	public void DoCommand() throws Exception{
		...
		String GOODS_NO=getRequest().getParameter("GOODS_NO");
		...
		if(Checks()&&bool){
			if(SysUtility.isEmpty(allid)){
				...
				if (SysUtility.isEmpty(Indx)) {
					//得到大于当前序号+1的jsonObject
					JSONObject jsonObject=getDataAccess().GetTableJSON("goodsList", "select * from ITF_DCL_IO_DECL_GOODS where DECL_NO='"+pIndx+"' and GOODS_NO >=" + (Integer.parseInt(GOODS_NO)+1)+ "order by GOODS_NO" );
					...
					SaveToTable("GoodsData", "GOODS_NO",String.valueOf((Integer.parseInt(GOODS_NO)+1)));//做插入操作的序号+1
					...
					if(!getDataAccess().Insert("ITF_DCL_IO_DECL_GOODS", getFormDatas().getJSONArray("GoodsData"),"GOODS_ID")){
						...
						String[] ps = new String[]{ Indx , GetDataValue("GoodsData", "GOODS_NO") , GetDataValue("GoodsData", "DECL_NO") };
						getDataAccess().ExecSQL(SQL, ps);
						//遍历大于当前序号+1的list
						JSONObject mainGoodsData = new JSONObject();
						JSONArray tempArrs=new JSONArray();
						boolean updateFlag=false;
						if(SysUtility.isNotEmpty(jsonObject.getJSONArray("goodsList")) && jsonObject.getJSONArray("goodsList").length()>0){
							for (int k = 0; k < jsonObject.getJSONArray("goodsList").length(); k++) {
								 //执行完清空
								 tempArrs=new JSONArray();
								 mainGoodsData=new JSONObject();
								 mainGoodsData= jsonObject.getJSONArray("goodsList").getJSONObject(k);
								 mainGoodsData.put("GOODS_NO",Integer.parseInt((String)mainGoodsData.get("GOODS_NO"))+1);
								 tempArrs.put(mainGoodsData);
								 updateFlag=getDataAccess().Update("ITF_DCL_IO_DECL_GOODS", tempArrs,"GOODS_ID");
								 if(!updateFlag){
									 break;
								 }
							}
							 ...
							 mainGoodsData=new JSONObject();
							if(updateFlag){
								 ReturnMessage(true, "添加成功!");
							 }else{
								 ReturnMessage(false, "添加失败!");
							 }
						}else{
