...
@WebServlet("/forms/DeleteInspAtts")
public class DeleteInspAtts extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;
	public DeleteInspAtts()
	{
	...
	public void DoCommand() throws Exception{
		...
		if(rows > 0){
			...
			for(int i = 0 ; i < rows ; i ++){
				boolean isFlag =getDataAccess().ExecSQL("delete from  ITF_DCL_IO_DECL_ATT where ATT_ID = ? " ,new String[]{ this.GetDataValue("key", "ATT_ID", i)});
				if(isFlag){
					StatusCount++;
				}				
			}
