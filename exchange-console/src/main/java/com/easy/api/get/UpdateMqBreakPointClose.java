...
@WebServlet("/UpdateMqBreakPointClose")
public class UpdateMqBreakPointClose extends MainServlet {
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    if("0".equals((String)map.get("is_breakpoint"))) {
			    	ErrorCount++;
			    	continue;
			    }
			    boolean falg=getDataAccess().ExecSQL("update exs_config_xmltomq set IS_BREAKPOINT='0' where INDX='"+INDX+"'");
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
