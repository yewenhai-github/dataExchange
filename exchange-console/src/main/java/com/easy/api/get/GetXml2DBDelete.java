...
@WebServlet("/GetXml2DBDelete")
public class GetXml2DBDelete extends MainServlet {
	private static final long serialVersionUID = -510507058071806803L;
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from EXS_CONFIG_XMLTODB where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update EXS_CONFIG_XMLTODB set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
