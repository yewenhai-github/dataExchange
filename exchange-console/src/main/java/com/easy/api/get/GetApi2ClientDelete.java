...
@WebServlet("/GetApi2ClientDelete")
public class GetApi2ClientDelete extends MainServlet {
	private static final long serialVersionUID = -4933226252868326225L;
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from exs_config_api_push where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update exs_config_api_push set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
