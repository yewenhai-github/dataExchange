...
@WebServlet("/GetDB2XmlDelete")
public class GetDB2XmlDelete extends MainServlet {
	private static final long serialVersionUID = -3357273816201860669L;
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from exs_config_dbtoxml where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update exs_config_dbtoxml set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
