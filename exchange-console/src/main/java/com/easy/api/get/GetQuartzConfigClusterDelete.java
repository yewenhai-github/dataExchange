...
@WebServlet("/GetQuartzConfigClusterDelete")
public class GetQuartzConfigClusterDelete extends MainServlet {
	private static final long serialVersionUID = 3853718947704861704L;
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from exs_quartz_config_CLUSTER where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update exs_quartz_config_CLUSTER set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
