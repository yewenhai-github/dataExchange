...
@WebServlet("/SaveApiServerByIndx")
public class SaveApiServerByIndx extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;

	public void DoCommand() throws Exception {
		String indx = GetDataValue("XmlData", "INDX");  
		if (!SysUtility.isEmpty(indx)) {
			Datas datas = getDataAccess().GetTableDatas("TEMP", "select * from exs_config_api_polling where indx = ?", indx);
		...
		SaveToDB("XmlData", "exs_config_api_polling");
