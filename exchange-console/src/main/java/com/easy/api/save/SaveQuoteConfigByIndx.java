...
@WebServlet("/SaveQuoteConfigByIndx")
public class SaveQuoteConfigByIndx extends MainServlet {
	private static final long serialVersionUID = 1564067536519132801L;

	public void DoCommand() throws Exception {
		String indx = GetDataValue("XmlData", "INDX");  
		if (SysUtility.isNotEmpty(indx)) {
			Datas datas = getDataAccess().GetTableDatas("TEMP", "select * from exs_quote_config where indx = ?", indx);
		...
		SaveToDB("XmlData", "exs_quote_config");
