...
@WebServlet("/DeleteApi2ServerQuartz")
public class DeleteApi2ServerQuartz extends MainServlet {
	private static final long serialVersionUID = 974126302418179391L;
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from exs_config_api_polling where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update exs_config_api_polling set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
