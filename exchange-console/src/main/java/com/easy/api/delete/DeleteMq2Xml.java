...
@WebServlet("/DeleteMq2Xml")
public class DeleteMq2Xml extends MainServlet {
	private static final long serialVersionUID = -7221955608513981621L;
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from exs_config_mqtoxml where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update exs_config_mqtoxml set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
