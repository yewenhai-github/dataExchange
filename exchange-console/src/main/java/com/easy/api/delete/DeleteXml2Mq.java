...
@WebServlet("/DeleteXml2Mq")
public class DeleteXml2Mq extends MainServlet {
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update exs_config_xmltomq set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
