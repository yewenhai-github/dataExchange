...
@WebServlet("/DeleteQuoteConfig")
public class DeleteQuoteConfig extends MainServlet {
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from exs_quote_config where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("delete from exs_quote_config where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
