...
@WebServlet("/DeleteXml2Split")
public class DeleteXml2Split extends MainServlet {
	private static final long serialVersionUID = 4369486397988293547L;
	...
	public void DoCommand() throws Exception{
		...
		try {
			...
			String sqlent = "select * from exs_config_xmltosplit where 1=1 "+conditions;
			...
			for (int i = 0; i < listent.size(); i++) {
			    ...
			    boolean falg=getDataAccess().ExecSQL("update exs_config_xmltosplit set IS_ENABLED='0' where INDX='"+INDX+"'");	
				if(falg){					
					SuccessCount++;
				}else{
					ErrorCount++;
